...
turbulent values, we repurpose its sub-$O(1)$ range---inert in the baseline
...
distinction is in \emph{what the field represents}. A simple scaling argument shows that the sub-$O(1)$ range of $\tilde\nu$ is the
...
This is precisely the part of the SA state space the baseline model barely uses:
for $\chi\!\lesssim\!1$ the field contributes negligibly to the eddy viscosity and
is dynamically inert \cite{spalart_allmaras_1994}. The natural amplitude and
length scales of laminar instabilities thus correspond to a range of $\tilde\nu$
that SA leaves idle---a coincidence we exploit: we let that range \emph{be} the
...
classical $O(0.15$--$0.2)$~\cite{michalke_1964}; we identify $a_{\max}$
