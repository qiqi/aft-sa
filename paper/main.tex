...
Section~\ref{sec:impl} notes the solver and how the model is seeded. The model
is then exercised across three pressure-gradient regimes of increasing
complexity.
...
Gating the destruction by $\sigma_t$ is likewise
...
\section{Solver and seeding}\label{sec:impl}

The results below were produced with Flow360, an unstructured, second-order
finite-element RANS solver with a Roe flux; the numerical scheme is otherwise
irrelevant to the model and we do not dwell on it. The only change from the
baseline SA model is the production/destruction of Eq.~\eqref{eq:blend}, and
because the amplification rate depends only on the local mean flow
(Sec.~\ref{sec:model}), the modified source keeps an analytic Jacobian and is
integrated exactly as the baseline turbulence equation is.

Two points of \emph{usage} bear on the model itself. First, the freestream
modified-viscosity ratio $\chi_\infty$ sets both the SA freestream value and the
disturbance seed (Sec.~\ref{sec:calib}). Second, the interior $\tilde\nu$ field is
initialized at this laminar level $\chi_\infty$ rather than the solver's default
near-turbulent value: since SA does not relaminarize, a turbulent initial field
would persist from the leading edge and bypass the transition model.
...
of the soft handover \eqref{eq:sigma}. The
...
per level. The freestream modified-viscosity ratio is set to the $N_\mathrm{crit}\!=\!9$
anchor, $\chi_\infty\!=\!c_{v1}e^{-9}\!\approx\!8.76\!\times\!10^{-4}$
(Sec.~\ref{sec:calib}).
...
$\approx\!1\!\times\!10^{-3}$ at $\alpha\!=\!15^\circ$) at L2---is therefore a
...
the front. A convex blend $\sigma_t P_\mathrm{SA}+(1-\sigma_t)P_\mathrm{AFT}$
would leave a $(1-\sigma_t)P_\mathrm{AFT}$ bleed there, keeping the
...
$\max$ form removes it: once $\sigma_t P_\mathrm{SA}$ overtakes amplification just
...
\begin{table}[tp]
  ...
  stall onset. The present computation and the NASA oil-flow
...
externally). Because the amplification rate depends only on the local mean flow
and not on the working variable, the source is local and analytically
differentiable, like the baseline SA production it replaces.
...
or below $0.01\,c$, and the two independently-generated mesh families agree to
