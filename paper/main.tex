...
(log); \emph{row 2} the pressure-gradient/profile indicator being calibrated in
...
NLF (favorable-PG tuning) this is the streamwise pressure-gradient parameter
$\lambda_p$ of Eq.~\eqref{eq:indicators} (reference lines at
$\lambda_\ast\!=\!0.64$ and $0$), and for the Eppler~387 (adverse-PG tuning) it
...
response the Eppler~387 calibrates: on the upper surface $\Gamma$ climbs from
...
cliff/rate factors calibrated on the NLF transfer to the Eppler~387 without
retuning and leave the bubble-controlling amplification untouched.
