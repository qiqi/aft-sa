...
field is introduced. The amplification rate depends only on the local mean
flow---a vorticity Reynolds number and a profile-fullness indicator---and its
kernel is anchored to canonical stability limits: the free-shear rate, the
Blasius $e^N$ envelope, and the Falkner--Skan pressure-gradient response. The
model is then tested on three cases spanning zero, favorable, and adverse
pressure gradients---a flat plate, the NLF(1)-0416 airfoil at
$Re\!=\!4\!\times\!10^6$, and the Eppler 387 at $Re\!=\!2\!\times\!10^5$---with
...
transition from within, rather than having it triggered by a separate marker.
...
model is its Yin: transition arises autogenously, from a variable the baseline
equation already carries, at essentially no added cost. Here ``AI'' abbreviates
\emph{Autogenous Inception}, not the other kind.} (Spalart--Allmaras with
\emph{A}utogenous \emph{I}nception), distinguishing it from Coder's SA-AFT, in
which a separate amplification factor is transported. The contribution is thus a
...
\begin{table}[t]
  ...
  \end{tabular}
\end{table}
SA-AI is scoped to natural (Tollmien--Schlichting) transition.
...
growth ceiling, the Blasius $e^N$ envelope, Drela's critical Reynolds number, and
Mack's freestream-turbulence receptivity---with a single favorable-gradient
onset-delay constant the only quantity fit to the transition cases.
Section~\ref{sec:impl} details the numerical implementation, including the
analytic source Jacobian. The model is then exercised across three
pressure-gradient regimes of increasing complexity.
...
the natural-transition $Tu$ range, confirming the Blasius envelope and onset.
Section~\ref{sec:nlfval} uses the NLF(1)-0416 natural-laminar-flow airfoil to
set the one favorable-gradient onset-delay constant $K_\lambda$ against the
$e^N$ rooftop ignition and to test the favorable-PG handover.
Section~\ref{sec:eppval} uses the Eppler 387 at low Reynolds number to test the
adverse-pressure-gradient, laminar-separation-bubble regime, with no further
constant. The same unified
...
\begin{equation}
  ...
  \label{eq:rate}
\end{equation}
with $a\equiv0$ for $Re_\Omega\le Re_\Omega^{\mathrm{c}}$, and $S$ the logistic
function. The argument $z$ superposes two physically distinct decisions. The
...
\emph{above} the attached-layer asymptote $\Gamma\!=\!1$, so an attached
zero-pressure-gradient profile sits on the low shoulder of the sigmoid and only
fuller, inflectional (near-separation) profiles reach the top. The log-barrier
...
the viscous sublayer automatically---and ever more sharply under mesh
refinement, where $\Gamma\!\to\!1$ is a generic near-wall limit, not a true
instability. Second---and this is the one modeling decision we make about
...
postponing the start of growth; it leaves the growth rate itself untouched.
Favorable-gradient stabilization is therefore an \emph{onset delay}, not a rate
suppression---a distinction we defend on stability-theory grounds and calibrate
in Sec.~\ref{sec:calib}. Both terms read only the local mean flow, never
$\tilde\nu$, so the rate carries an analytic Jacobian (Sec.~\ref{sec:impl}).
Figure~\ref{fig:kernel} maps the kernel.

\begin{figure}[tp]
  \centering
  \includegraphics[width=0.98\textwidth]{kernel_maps.pdf}
  \caption{The amplification-rate kernel of Eq.~\eqref{eq:rate}, in full.
...
\paragraph{Favorable-pressure-gradient cliff.} The favorable-gradient effect
enters through the threshold and nowhere else: $Re_\Omega^{\mathrm{c}}$ rises
with $\lambda_p$, so an accelerating layer must grow thicker before amplification
is permitted, postponing the onset of growth:
\begin{equation}
...
factor $c_{\nu,\mathrm{aft}}\!<\!1$ (its value fixed in Sec.~\ref{sec:calib}),
\begin{equation}
  (\nu+\tilde\nu)\;\longrightarrow\;(c_{\nu,\mathrm{aft}}\,\nu+\tilde\nu)
...
This reduction leaves Spalart's turbulent calibrations intact: it touches only
the molecular part of the diffusion, which is negligible in every region where
those high-Reynolds calibrations were set, so the mean flow does not feel it. The
region-by-region accounting, and the resulting sub-percent skin-friction margin,
are deferred to Sec.~\ref{sec:calib}.

\subsection{Assembled one-equation model}
...
canonical stability limits. The favorable-gradient onset-delay $K_\lambda$ is
anchored in the same spirit---its form fixed by requiring Falkner--Skan profiles
to ignite at their Drela critical Reynolds number (Fig.~\ref{fig:collapse})---but
its value carries one deliberate design choice and is confirmed against the
NLF(1)-0416 rooftop (Sec.~\ref{sec:nlfval}); it is the closest thing to a fitted
constant. The subsequent cases are thus predictive tests.
...
The favorable-gradient onset-delay slope $K_\lambda$ sets how fast the cliff
$Re_\Omega^{\mathrm{c}}(\lambda_p)$ rises with acceleration; its form is anchored
by requiring the cliff to place amplification onset at the stability-theory
critical Reynolds number across pressure gradients. Figure~\ref{fig:collapse}
traces Falkner--Skan profiles---from Blasius through strong favorable gradient
($\beta\!>\!1$)---each placed at its \emph{own} Drela--Giles critical Reynolds
number $Re_{\theta c}(H)$, in the normalized
$(Re_\Omega/Re_\Omega^{\mathrm{c}},\Gamma)$ plane of the kernel. With
$Re_\Omega^{\mathrm{f}}\!=\!100$ and $K_\lambda\!=\!10$ every profile ignites just
past the cliff at a small amplification rate: growth turns on right at the
critical Reynolds number, uniformly from the flat plate to strong acceleration,
even though $Re_{\theta c}$ itself rises about $40\times$ across this family
(from $242$ at Blasius, $H\!=\!2.59$, to $\sim\!9800$ at $\beta\!=\!1.8$,
$H\!=\!2.17$). This is the sense in which the cliff is anchored to Drela's
envelope $e^N$ method~\cite{drela_giles_1987}: the favorable-gradient response is
carried by the critical Reynolds number, not the growth rate---which changes only
mildly over the same range---so no separate rate factor is needed.

One feature of Figure~\ref{fig:collapse} is deliberate: the low-Reynolds Blasius
profile ignites at slightly \emph{higher} amplification than the accelerated
ones. Blasius transitions at small $Re_\theta$, where the layer is thin and the
reduced but nonzero laminar diffusion ($c_{\nu,\mathrm{aft}}\!=\!1/12$) is
relatively stronger, so a little more amplification is needed for net growth; the
favorable-gradient profiles transition at much larger $Re_\theta$ and need less.
This trend---more amplification at low $Re_\theta$, less at high---is a design
choice rather than a fine calibration. The resulting $K_\lambda\!=\!10$ is
consistent with the NLF(1)-0416 $\alpha\!=\!0^\circ$ rooftop, where mfoil's $e^N$
method holds $N\!\approx\!0$ until $x/c\!\approx\!0.2$.

\begin{figure}[tp]
  \centering
  \includegraphics[width=0.78\textwidth]{collapse_cliff.pdf}
  \caption{The onset cliff anchored to Drela + Falkner--Skan. Falkner--Skan
  profiles from Blasius ($\beta\!=\!0$) through strong favorable gradient
  ($\beta\!=\!1.8$, $m\!=\!9$), each placed at its own Drela--Giles critical
  Reynolds number $Re_{\theta c}(H)$, traced in the normalized
  $(Re_\Omega/Re_\Omega^{\mathrm{c}},\Gamma)$ plane of the amplification kernel
  $S(z)$ (shading; cliff at ratio $1$, dotted). Stars mark each profile's peak
  amplification. With $Re_\Omega^{\mathrm{f}}\!=\!100$ and $K_\lambda\!=\!10$
  every profile ignites just past the cliff at a small rate---amplification turns
  on at the critical Reynolds number across the whole pressure-gradient family.
  The low-$Re_\theta$ Blasius case is deliberately the hottest (thin layer,
  relatively stronger $c_{\nu,\mathrm{aft}}\nu$ diffusion).}
  \label{fig:collapse}
\end{figure}
...
The value $c_{\nu,\mathrm{aft}}\!=\!1/12$ is the reduction that restores a
$dN/dRe_\theta$ in the Drela envelope range, and it is invisible to Spalart's
turbulent calibrations. Those constants
($c_{b1},c_{b2},c_{w1},\sigma,\kappa,c_{v1}$) were fixed on high-Reynolds
turbulent flows---free-shear spreading rates and flat-plate skin friction---and
the scaling \eqref{eq:nulamscale} leaves each intact, by a region-by-region
partition:
\begin{enumerate}
...
skin friction $C_f$, well below the $2$--$5\%$ experimental scatter of the
flat-plate data on which SA was founded and below typical industrial
...
number~\cite{drela_giles_1987}; as established above, its slope $K_\lambda$ is
anchored to Drela's $Re_{\theta c}(H)$ through the Falkner--Skan map
$H(\lambda_p)$, fixing the accelerating-flow stabilization against stability
theory rather than the airfoil data alone.
...
limit of the one-equation formulation.
