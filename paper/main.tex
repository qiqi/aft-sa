...
\begin{table}[t]
  ...
  \end{tabular}
\end{table}
SA-AF is scoped to natural (Tollmien--Schlichting) transition. Its amplification
...
Eq.~\eqref{eq:paft},
\begin{equation}
  a(Re_\Omega,\Gamma,\lambda_p) = a_{\max}\,S(z)\,\sigma_\mathrm{FPG}(\lambda_p),
  \quad
  z = s(\Gamma-g_c) + \ln\!\Big(1-\big(Re_\Omega^{\mathrm{c}}/Re_\Omega\big)^{p}\Big),
...
growth onset matches Drela's $Re_{\theta,\mathrm{crit}}\!\approx\!200$; the cliff
is $\Gamma$-independent (favorable-gradient onset delay is carried by $\lambda_p$
below, not by tilting the cliff). Because $Re_\Omega\!=\!d^2\omega/\nu\!\to\!0$
...
true instability. The prefactor $\sigma_\mathrm{FPG}(\lambda_p)$ suppresses
amplification in favorable pressure gradient,
\begin{equation}
  \sigma_\mathrm{FPG}(\lambda_p) =
       \frac{1}{1+\exp(s_\lambda(\lambda_p-\lambda_\ast))} ,
  \label{eq:fpg}
\end{equation}
anchored to Falkner--Skan profiles at $\eta\!\approx\!1$ (the
kernel-active band): Blasius gives $\lambda_p\!=\!0$ and Hiemenz ($\beta\!=\!1$)
$\lambda_p\!\approx\!1.29$, so anchoring $\sigma_\mathrm{FPG}(0)\!=\!0.95$ and
$\sigma_\mathrm{FPG}(1.29)\!=\!0.05$ gives $(\lambda_\ast, s_\lambda)\!=\!(0.64,
4.56)$; an adverse gradient ($\lambda_p\!<\!0$) sees
$\sigma_\mathrm{FPG}\!>\!0.95$ and leaves the inflectional-profile amplification
the $\Gamma$-sigmoid already captures untouched. All three factors depend only on
the local mean flow, not on $\tilde\nu$, so the rate \eqref{eq:rate} preserves
the analytic Jacobian of Sec.~\ref{sec:impl}. Figure~\ref{fig:kernel} maps the
two shape factors of the rate.

\begin{figure}[tp]
  \centering
  \includegraphics[width=\textwidth]{kernel_maps.pdf}
  \caption{The two shape factors of the amplification rate \eqref{eq:rate}.
  \emph{Left}: the $\Gamma$-sigmoid-with-cliff $S(z)$ over $(Re_\Omega,\Gamma)$
  at $\lambda_p\!\le\!0$ (so $\sigma_\mathrm{FPG}\!=\!1$): the log barrier zeroes
  the rate below the cliff floor $Re_\Omega^{\mathrm{f}}\!=\!100$ (dotted), while
  above it the sigmoid centered at $g_c\!=\!1.572$ (dashed) leaves attached
  Blasius-like profiles ($\Gamma\!\approx\!1$) on the low shoulder and saturates
  on inflectional profiles ($\Gamma\!\to\!2$). \emph{Right}: the
  favorable-pressure-gradient suppression $\sigma_\mathrm{FPG}(\lambda_p)$,
  anchored to $0.95$ on Blasius ($\lambda_p\!=\!0$) and $0.05$ on Hiemenz
  stagnation flow ($\lambda_p\!=\!1.29$), with center $\lambda_\ast\!=\!0.64$.}
  \label{fig:kernel}
\end{figure}
...
This rate/onset split mirrors how Drela's envelope $e^N$
method~\cite{drela_giles_1987} responds to a favorable gradient, which lets us
check both factors against stability theory rather than treat them as free.
Along the Falkner--Skan family the shape factor falls monotonically as the
gradient strengthens ($H\!=\!2.59\!\to\!2.22$ from Blasius to Hiemenz
$\beta\!=\!1$, computed from the self-similar profiles), and the local indicator
tracks it ($\lambda_p\!=\!0\!\to\!1.29$), so Drela's two $H$-dependences become
functions of $\lambda_p$. His growth rate $d\tilde N/dRe_\theta(H)$ weakens as
the profile fills---the behavior $\sigma_\mathrm{FPG}$ encodes---while the
critical Reynolds number $Re_{\theta c}(H)$ rises steeply, by $\approx\!27\times$
from Blasius to Hiemenz: this is the onset delay carried by the cliff. In the
weak-gradient regime that sets airfoil rooftops the logarithmic slope
$d\ln Re_{\theta c}/d\lambda_p\!\approx\!6.5$ is comparable to the fitted
$K_\lambda\!=\!10$, so the one favorable-gradient constant is consistent with the
$e^N$ critical Reynolds number rather than an independent tuning. Together the
rate factor and the cliff reproduce the favorable-gradient stabilization that the
$e^N$ method distributes between $d\tilde N/dRe_\theta$ and $Re_{\theta c}$.
...
Collecting the three modifications, the SA-AF model solves the single transport
equation
\begin{equation}
  \frac{D\tilde\nu}{Dt} = P - D
  + \frac{1}{\sigma}\nabla\!\cdot\!\big[(c_{\nu,\mathrm{aft}}\,\nu+\tilde\nu)\,\nabla\tilde\nu\big]
...
favorable-pressure-gradient parameters $(\lambda_\ast,s_\lambda,K_\lambda)$.
...
The favorable-pressure-gradient response $\sigma_\mathrm{FPG}(\lambda_p)$ in
particular plays the role that the Thwaites/Pohlhausen pressure-gradient
parameter plays in the Abu-Ghannam \& Shaw onset
correlation~\cite{abu_ghannam_shaw_1980} and that the shape factor $H$ plays in
Drela's $e^N$ envelope~\cite{drela_giles_1987}; anchoring it to the exact
self-similar Falkner--Skan profiles (Blasius at $\lambda_p\!=\!0$, Hiemenz at
$\lambda_p\!\approx\!1.29$; Sec.~\ref{sec:model}) fixes the accelerating-flow
stabilization from stability theory rather than from the airfoil data.
...
adopt it wholesale rather than fit it. This is legitimate because the SA-AF
...
\begin{table}[t]
  ...
  $(\lambda_\ast,s_\lambda)$ anchor $\sigma_\mathrm{FPG}$ to the Falkner--Skan
  family. The Tu$\to\chi_\infty$ receptivity closure (Eq.~\eqref{eq:tumap}) is
...
$\lambda_p$ of Eq.~\eqref{eq:indicators} (reference lines at
$\lambda_\ast\!=\!0.64$ and $0$), and for the Eppler~387 (adverse-PG regime) it
...
\begin{figure}[tp]
  ...
  SA-AF prediction (structured O-grid and unstructured meshes, all three
...
(Fig.~\ref{fig:nlfpolar}). On the refined meshes the SA-AF polar tracks the
...
mesh families agreeing to within $\Delta C_d\!\approx\!5\times10^{-4}$ at L2.
...
favorable-gradient segment near the leading edge, so the $\sigma_\mathrm{FPG}$
cliff/rate factors ($K_\lambda$ fit on the NLF, the rest anchored to
Falkner--Skan) transfer to the Eppler~387 without retuning and leave the
bubble-controlling amplification untouched.
...
\begin{figure}[tp]
  ...
  SA-AF prediction (finest structured O-grid and unstructured meshes, L2) at
...
Figure~\ref{fig:epppolar} overlays the SA-AF polar on the measured section
...
number ($Re_\Omega^{\mathrm{f}}$), and the Falkner--Skan pressure-gradient
response ($\sigma_\mathrm{FPG}$)---and then exercised across three
