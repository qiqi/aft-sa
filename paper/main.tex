...
$\chi$ (left) and the resulting $C_f(Re_\theta)$ (right); both draw black
curves with a distinct symbol per $Tu$, and both mark the Schubauer--Skramstad
experimental transition with a solid vertical line and a filled symbol on the
bottom edge --- at the Blasius-mapped location $x_\mathrm{SS}\!=\!(Re_{\theta,
\mathrm{SS}}/0.664)^2\,\nu/U_\infty$ on the left and at
$Re_\theta^\mathrm{tr}$ on the right
($Re_\theta^\mathrm{tr}\!=\!1115,\,890,\,681,\,575,\,345$). The operational
transition --- the $C_f$ rise --- occurs where $\chi$ reaches the
eddy-viscosity half-saturation value $c_{v1}\!=\!7.1$, not where the
production blend first engages at $\chi\!=\!1$ (both drawn as horizontal
references on the bottom-left panel); the experimental verticals accordingly
line up with the simulation's $\chi\!=\!c_{v1}$ crossings rather than its
$\chi\!=\!1$ crossings. The $C_f$ collapses onto the laminar Blasius curve in
the pre-transitional region and onto the turbulent correlation once $\chi$ has
saturated. The
...
\begin{figure}[tp]
  ...
  Schubauer--Skramstad~\cite{schubauer_skramstad_1948} levels
  $Tu\!=\!0.026$--$0.85\%$ (rows). \emph{Left columns}: contours of
  $u/U_\infty$ and $\log_{10}\hat\nu$ in the $(Re_x,Re_y)$ plane.
  \emph{Bottom-left}: in-BL maximum $\chi\!=\!\tilde\nu/\nu$ vs $x$, with
  horizontal references at $\chi\!=\!1$ and $\chi\!=\!c_{v1}\!=\!7.1$.
  \emph{Bottom-right}: $C_f$ vs $Re_\theta$ with the laminar
  ...
  verticals and filled bottom-edge symbols mark the Schubauer--Skramstad
  transition. See text for the $c_{v1}$ transition criterion, the calibration
  fit, and why the experimental markers align with the $\chi\!=\!c_{v1}$
  crossings.}
