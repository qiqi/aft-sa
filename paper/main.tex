...
are pinned against the Schubauer--Skramstad transition Reynolds numbers across
five turbulence levels, fixing the Blasius behavior. On the NLF(1)-0416
...
the Schubauer--Skramstad transition data at five $Tu$ levels (the Blasius
behavior). Section~\ref{sec:nlfval} uses the NLF(1)-0416 natural-laminar-flow
...
\begin{figure}[tp]
  \centering
  \includegraphics[width=0.95\textwidth]{flat_plate_batch_flow360.pdf}
  ...
  $Tu\!=\!0.026\%,\,0.06\%,\,0.18\%,\,0.30\%,\,0.85\%$ (rows). \emph{Left columns}:
  contours of streamwise velocity $u/U_\infty$ and of $\log_{10}\hat\nu$ in the
  $(Re_x,Re_y)$ plane. \emph{Bottom-left}: in-BL maximum $\chi\!=\!\tilde\nu/\nu$
  along $x$, one curve per $Tu$, with colored dashed verticals at the
  Schubauer--Skramstad transition locations $x_\mathrm{SS}\!=\!(Re_{\theta,SS}/0.664)^2\nu/U_\infty$.
  The two horizontal grey references mark $\chi\!=\!1$ (start of the $\sigma_t$
  blend) and $\chi\!=\!c_{v1}\!=\!7.1$ (the half-saturation of $f_{v1}$, where
  $C_f$ actually rises). The experimental verticals line up with the simulation's
  $\chi\!=\!c_{v1}$ crossings, not the $\chi\!=\!1$ crossings: the operational
  transition in the model is the $C_f$ rise, which occurs at $\chi\!\approx\!c_{v1}$,
  not at the onset of $\sigma_t$ engagement. \emph{Bottom-right}: $C_f(Re_\theta)$
  against the laminar Blasius correlation $C_f\!=\!0.441/Re_\theta$ (dashed) and
  the turbulent log-law correlation $C_f\!=\!2[(\ln Re_\theta)/0.38+3.7]^{-2}$ (dotted).
  Vertical gray reference lines and filled markers along the bottom edge indicate
  the Schubauer--Skramstad experimental $Re_\theta^\mathrm{tr}$ at each $Tu$;
  the simulated $C_f$ leaves the laminar branch and joins the turbulent
  correlation slightly downstream of each marker (within $\pm12\%$ on $Re_\theta$
  across the four natural-transition cases). The baseline SA model cannot produce
  this behavior: it bypasses to turbulence at the leading edge for any nonzero
  freestream seed. As $Tu$ rises the seed
  $\chi_\infty\!\sim\!e^{-N_\mathrm{crit}}$ grows, so fewer $e$-folds are needed
  to reach the half-saturation point and transition moves upstream.}
...
\begin{figure}[tp]
  ...
  $Re\!=\!4\!\times\!10^6$, $\chi_\infty\!=\!8.76\!\times\!10^{-4}$. \emph{Row 1:}
  wall-normal-probe maximum of $Re_\Omega$ (log axis). \emph{Row 2:}
  wall-normal-probe minimum of $\sigma_\mathrm{FPG}$ --- the
  favorable-pressure-gradient suppression of Eq.~\eqref{eq:fpg} evaluated at
  the wall-normal location of strongest favorable PG in the BL. This is the
  parameter directly tuned by the present section: $\sigma_\mathrm{FPG}\!\to\!0$
  over the favorable-gradient rooftop (no amplification) and rises to
  $\sigma_\mathrm{FPG}\!\to\!1$ once recovery begins. \emph{Row 3:} in-BL
  maximum modified-viscosity ratio $\chi\!=\!\max_y\tilde\nu/\nu$ on a log
  ...
  $x_\mathrm{tr}$ on each surface. \emph{Row 4:} $-C_p$. \emph{Row 5:} signed
  $C_f$. Color encodes the surface (upper blue, lower red); line style encodes
  the mesh family (structured O-grid: solid, unstructured cavity: dashed);
  line thickness encodes the refinement level (L0 thinnest, L2 thickest).
  Dotted curves are the mfoil $e^9$ reference. Surfaces are separated by
  walking the airfoil contour via mesh connectivity and splitting at the
  geometric LE/TE extrema; this is essential on the NLF because the reflex
  camber places the lower surface above the chord line over the aft region.
  At $\alpha\!=\!0^\circ$ both surfaces transition near $x/c\!\approx\!0.45$,
  ...
  near design. The transition locations track the $e^9$ envelope to within
  $\sim\!0.03$--$0.05\,c$ on both surfaces, both meshes, and at every
  refinement level from L0 ($N{\approx}200$ surface points) to L2 ($N{=}800$).}
  \label{fig:nlfcflow}
\end{figure}
...
\begin{figure}[tp]
  \centering
  \includegraphics[width=0.95\textwidth]{eppler_cf_lowalpha.pdf}
  \caption{Eppler 387 surface distributions at $\alpha\!\in\!\{0^\circ,2^\circ\}$,
  $Re\!=\!2\!\times\!10^5$, $M\!=\!0.1$, $\chi_\infty\!=\!8.76\!\times\!10^{-4}$
  (matched to $N_\mathrm{crit}\!=\!9$). \emph{Row 1}:
  wall-normal-probe maximum of $Re_\Omega$ (log axis). \emph{Row 2}:
  wall-normal-probe minimum of $\sigma_\mathrm{FPG}$ (the
  favorable-pressure-gradient suppression of Eq.~\eqref{eq:fpg} at the
  wall-normal location of strongest favorable PG): $\sigma_\mathrm{FPG}\!\to\!0$
  in regions where the FPG actively suppresses amplification, rising back
  toward $1$ once the recovery begins. The Eppler 387 at low Re carries only
  a short FPG segment near the LE, so $\sigma_\mathrm{FPG}\!\to\!1$ over most
  of the chord and the parameter tuned on the NLF transfers cleanly without
  retuning. \emph{Row 3}: in-BL maximum
  modified-viscosity ratio $\chi\!=\!\max_y\tilde\nu/\nu$ on a log axis (left),
  with mfoil's amplification factor $N$ on the matched linear axis (right) and
  the vertical dotted lines at mfoil's $x_\mathrm{tr}$. \emph{Row 4}:
  $-C_p$. \emph{Row 5}: \emph{signed} $C_f$ (positive downstream, negative
  in reversed flow): the LSB recirculation appears directly as the dip below
  zero on the upper surface. Color: upper (blue), lower (red). Line style:
  structured O-grid (solid), unstructured cavity (dashed). Line thickness: L0
  (thin), L1 (medium), L2 (thick). Dotted: mfoil $e^N$ at $N_\mathrm{crit}\!=\!9$.
  The bubble sits at $x/c\!\approx\!0.55$--$0.65$ at $\alpha\!=\!0^\circ$ and
  marches forward to $x/c\!\approx\!0.45$--$0.55$ at $\alpha\!=\!2^\circ$; both
  Flow360 and mfoil show the same recirculation structure.}
  \label{fig:eppcflow}
\end{figure}

\begin{figure}[tp]
  \centering
  \includegraphics[width=0.95\textwidth]{eppler_cf_highalpha.pdf}
...
and the $Tu\!\to\!\chi_\infty$ slope were pinned to the Schubauer--Skramstad
data at five $Tu$ levels, fixing the Blasius behavior. On the NLF(1)-0416 at
