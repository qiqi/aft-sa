...
Mack's freestream-turbulence receptivity---so that no constant is fit to the
transition cases (even the onset-delay slope $K_\lambda$ is derived, by matching
the cliff to the rate Drela's critical Reynolds number rises with gradient).
...
proportional to $Re_\theta$ ($\max_y Re_\Omega\!\approx\!2.2\,Re_\theta$, from the
Blasius profile) and is located at $y\!\approx\!\theta$, so $Re_\Omega$ inherits the role of $Re_\theta$
...
must be held off (it sets the onset-delay cliff introduced below). What
...
instance downstream of an adverse run that has since relaxed---so a profile
sitting at small local $\lambda_p$ can nonetheless be strongly unstable.
...
$\lambda_p$ it contains no integral and no memory---it simply reads the profile
shape that the upstream history has \emph{already produced}, which is the state
the instability responds to---so it plays locally the role the shape factor $H$
plays in envelope methods, without being $H$ itself. $\Gamma$ is built only from $\omega$, $d$, and the flow speed
...
Rather than fit these to the transition cases that follow, we anchor every one to
canonical stability limits. Even the favorable-gradient onset-delay $K_\lambda$
is \emph{derived}, not fit: it is set by matching the cliff's slope to the rate
at which Drela's critical Reynolds number rises with pressure gradient
(Eq.~\eqref{eq:klambda}, Figs.~\ref{fig:worstpoint}--\ref{fig:klambda}). The
model thus carries no constant tuned to a transition case, and the subsequent
airfoil cases are pure predictive tests.
...
layer. The link to the $e^N$ envelope is that the local rate $a\,\omega$
integrates along the layer to $N\!=\!\ln(\tilde\nu/\tilde\nu_\infty)$, so the
...
$(s,g_c)\!=\!(5.263,1.572)$. This is a calibration to the envelope, verified
numerically rather than asserted as an identity: integrating the disturbance
transport on the Blasius layer (Fig.~\ref{fig:nuhat}) recovers the full envelope,
$N$ accumulating to $\approx\!14$ over the plate---the established flat-plate
transition range.
...
$Re_{\theta,\mathrm{crit}}\!\approx\!200$. The low value is deliberate and
empirical: the floor merely \emph{permits} amplification, which the log-barrier
...
threshold precisely to offset the reduced-but-nonzero laminar diffusion
(Sec.~\ref{sec:nulamscale}) that otherwise damps the sub-$O(1)$ disturbance;
$100$ is a round, empirically chosen figure, not a finely tuned constant. The
cliff needs no
...
This pins $K_\lambda$ with no reference to the transition cases. As the gradient
turns weakly favorable ($\beta\!\to\!0^+$), the worst point's $\lambda_p$ rises at
a rate $d\lambda_p^{\mathrm{worst}}/d\beta\!\approx\!1.3$; the trigger's inboard
...
threshold at exactly the rate the critical Reynolds number rises---so that the
favorable-gradient response is carried by the onset delay, as in Drela's envelope
method~\cite{drela_giles_1987}, and not by a growth-rate change---gives
\begin{equation}
  K_\lambda = \left.\frac{d\ln Re_{\theta c}/d\beta}{d\lambda_p^{\mathrm{worst}}/d\beta}\right|_{\beta\to0}
  \approx \frac{12.3}{1.3} \approx 9.5.
  \label{eq:klambda}
\end{equation}
The worst point itself depends on $K_\lambda$, so this is a self-consistency
condition; Figure~\ref{fig:klambda} plots the implied $K_\lambda$ against the
value used, and the two meet at a fixed point $K_\lambda\!\approx\!9.7$. We adopt
the round value $K_\lambda\!=\!10$. The point is not the second digit---the slope
inputs are themselves ballpark, so $9.5$, $9.7$, and $10$ are indistinguishable
at that precision---but that the \emph{magnitude} of $K_\lambda$ is set by the
Drela--Giles envelope and the Falkner--Skan family rather than fit to the
transition cases. A more careful calibration of the coefficient is left to
future work.

\begin{figure}[tp]
  \centering
  \includegraphics[width=0.56\textwidth]{klambda_selfconsistent.pdf}
  \caption{Self-consistent determination of $K_\lambda$. Because the worst point
  moves with $K_\lambda$, the slope match of Eq.~\eqref{eq:klambda} is a fixed-point
  condition: the implied $K_\lambda$ (blue) equals the value used (dashed) at
  $K_\lambda\!\approx\!9.7$. The numerator, Drela's $d\ln Re_{\theta c}/d\beta
  \!\approx\!12.3$, is set by the Drela--Giles envelope through the Falkner--Skan
  map $H(\beta)$.}
  \label{fig:klambda}
\end{figure}
...
turbulent calibrations. The reason is structural: $c_{\nu,\mathrm{aft}}$ rescales
...
\begin{enumerate}
  ...
  equations feel.
\end{enumerate}
Propagated to the macroscopic solution, this $\sim\!15\%$ buffer-layer deficit
shifts the log-law intercept by only a small fraction of a percent in skin
friction $C_f$---safely below the $2$--$5\%$ experimental scatter of the
...
seed/threshold structure mirrors the $e^N$ envelope/$N_\mathrm{crit}$
...
plays the role of the $e^N$ threshold. Mack's slope $B$ therefore transfers to
the seed to the extent the SA-AI envelope is affine to the $e^N$ envelope over
this $Tu$ range---which the flat plate confirms to within $\sim\!8\%$
(Sec.~\ref{sec:flatplate}). In contrast, the $\gamma$--$Re_\theta$ and BC/BCM
...
transport equation; SA-AI instead borrows Mack's receptivity closure and derives
$K_\lambda$ from the Drela--Giles envelope. The flat
...
\begin{table}[t]
  ...
  Mack's $e^N$ correlation, adopted wholesale. The onset-delay slope $K_\lambda$
  is derived by matching the cliff to Drela's critical-Reynolds-number rise
  (Eq.~\eqref{eq:klambda}). All constants below
...
value~\cite{mack_1977}, adopted rather than fit; appreciable growth sets in near
$Re_\theta\!\approx\!200$, consistent with Drela's $Re_{\theta,\mathrm{crit}}$ for
Blasius (the soft cliff floor permits growth earlier but the gentle rate keeps it
negligible until then).
...
mesh families agreeing to within $\Delta C_d\!\approx\!6\times10^{-4}$ at L2.
...
$Re_\Omega^{\mathrm{c}}(\lambda_p)$ (with the derived $K_\lambda$) transfers to the
...
$0.10$--$0.15\,c$ upstream of mfoil's $e^9$ prediction at
$\alpha\!\in\!\{0^\circ,2^\circ,5^\circ\}$. This is the same bias seen on the NLF,
where it appears as $0.03$--$0.05\,c$ (Sec.~\ref{sec:nlfval}): a single, uniform
earlier-than-$e^9$ tendency whose origin is the handover structure itself---the
$C_f$ rise is registered at $\chi\!=\!1$, about $\ln c_{v1}\!\approx\!2$ $e$-folds
ahead of the nominal threshold $\chi\!=\!c_{v1}$ (Sec.~\ref{sec:calib}). The same
$\sim\!2$-$e$-fold shift maps to a larger $\Delta x/c$ on the lower-Reynolds
Eppler, where the layer develops more slowly. It is a uniform offset, absorbable
by a small shift of the $\chi_\infty$ anchor, not a per-case discrepancy.
...
per $10^4$ pseudo-steps. The front migrates through a marginally-amplifying band
where $a\,\omega$ is tiny, so its motion barely perturbs the global residual,
which is dominated by the already-converged bulk of the field. We accordingly stop each case by transition-location
...
two-stage restart reaches the same steady state far faster than
$f_\mathrm{slow}\!=\!0.01$ alone: the two routes agree in transition location to
$|\Delta x_\mathrm{tr}|\!<\!0.01\,c$ and in drag to $\sim\!10^{-4}$.
...
envelope, not fit---places growth onset at the $e^N$ rooftop ignition, and the
laminar-to-turbulent handover was predicted across
$\alpha\!\in\!\{0^\circ,4^\circ,9^\circ,15^\circ\}$ on
...
Several extensions follow. The constants adopted here are ballpark values
anchored to canonical limits and rounded for convenience (e.g.\
$K_\lambda\!=\!10$, $Re_\Omega^{\mathrm{f}}\!=\!100$), not the product of a
systematic optimization; a more careful calibration of the rate against
