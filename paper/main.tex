...
\begin{equation}
  ...
  \approx \frac{12.3}{1.3} \approx 10.
  \label{eq:klambda}
\end{equation}
The worst point itself depends on $K_\lambda$, so this is a self-consistency
condition; Figure~\ref{fig:klambda} plots the implied $K_\lambda$ against the value
used, and the two meet at $K_\lambda\!\approx\!9.7$. The committed
$K_\lambda\!=\!10$ is therefore \emph{derived} from the Drela--Giles envelope and
the Falkner--Skan family, not fit to the transition cases.
...
receptivity closure borrowed from Mack, and the favorable-gradient onset-delay
$K_\lambda$ itself derived from the Drela--Giles envelope (Eq.~\eqref{eq:klambda}),
\emph{no} scalar is fit to the transition cases. The flat
...
\begin{table}[t]
  ...
  (Eq.~\eqref{eq:klambda}); no constant is fit to the transition cases. All constants below
