...
\begin{tabbing}
  ...
  $c_{\nu,\mathrm{ai}}$ \> laminar-viscosity scale in the SA cross-stream diffusion only ($1/12$) \\
  ...
  $\sigma_t,\;\tau$ \> laminar-to-turbulent handover weight and its width \\
...
is a large fraction of the chord, and where it ends drives efficiency, range and
endurance, and acoustic signature, not drag alone.
...
TS-envelope stage can occupy most of the streamwise distance and largely determines
...
Mack's freestream-turbulence receptivity---so that the constants are calibrated on
simple, canonical flows rather than the transition test cases (even the onset-delay
slope $K_\lambda$ is set by matching the cliff to the rate at which Drela's critical
Reynolds number rises with gradient).
The model is exercised across three pressure-gradient regimes of increasing
complexity, all with the same solver and laminar seeding.
...
constants (collected in Sec.~\ref{sec:assembled}) are used across all three regimes and across the
...
\cite{abu_ghannam_shaw_1980}. Both parts are formed to be Galilean invariant. The
numerator is the material derivative $Dp/Dt$---the frame-independent rate of
...
unchanged, since fluid and wall translate together. With $d$, $\omega$, and
$\nabla p$ invariant to begin with, $\lambda_p$ and $\Gamma$ are frame
independent. It is positive in a favorable gradient
...
plays in envelope methods, without being $H$ itself.
...
fuller, inflectional (near-separation) profiles reach the top. The design intent
...
shear layer. Formed in that order---$Re_\Omega^{\mathrm{c}}$ first, never the
reciprocal of $\lambda_p$---the singular denominator raises no numerical issue.
...
\begin{equation}
  P = \max\!\big[(1-\sigma_t)\,P_\mathrm{AI},\;\sigma_t\,P_\mathrm{SA}\big], \qquad
  D = \sigma_t\,D_\mathrm{SA}.
  \label{eq:blend}
\end{equation}
The $\max$ hands the production cleanly from amplification to SA at the
crossover: for $\chi\!\le\!1$ ($\sigma_t\!=\!0$) it returns $P_\mathrm{AI}$,
...
factor $c_{\nu,\mathrm{ai}}\!<\!1$ (its value fixed in Sec.~\ref{sec:calib}),
\begin{equation}
  (\nu+\tilde\nu)\;\longrightarrow\;(c_{\nu,\mathrm{ai}}\,\nu+\tilde\nu)
...
region-by-region accounting, verified against a wall-layer solve, is deferred to
Sec.~\ref{sec:calib}.
...
favorable-pressure-gradient onset-delay slope $K_\lambda$. Each is set by a
canonical limit, not by the transition cases that follow: $a_{\max}$ by the
free-shear growth ceiling, $(s,g_c)$ by the Blasius $e^N$ envelope slope,
$Re_\Omega^{\mathrm{f}}$ by the onset Reynolds number, and $K_\lambda$ by the rate
at which Drela's critical Reynolds number rises with pressure gradient
(Eq.~\eqref{eq:klambda}, Fig.~\ref{fig:worstpoint}). None is
tuned to a transition case, so the airfoil cases remain predictive tests.

\subsection{The amplification-rate kernel}\label{sec:calib_rate}
...
$\omega_{i,\max}\!\approx\!0.19\,U_0/\delta$, while the peak vorticity is
$\omega_\mathrm{peak}\!=\!U_0/\delta$, so the vorticity-scaled rate is
$\omega_{i,\max}/\omega_\mathrm{peak}\!\approx\!0.19$. We identify $a_{\max}$
...
free mixing layer, and $a_{\max}$ is in any case a deliberately gentle envelope
rate (Sec.~\ref{sec:model}) that targets the onset location rather than the
growth rate itself.
...
$N$ accumulating to $\approx\!14$ across the established flat-plate transition
$Re_x$ range. The center sits near $g_c\!\approx\!1.5$ for a simple
reason---requiring the sigmoid to be fully on at $\Gamma\!=\!2$ and barely on at
$\Gamma\!=\!1$ forces it roughly midway between the two---and the pair was tuned
only against these two anchors.

The response at intermediate fullness was not itself calibrated, so it is a
genuine test. For each Falkner--Skan profile (indexed by its shape factor $H$) we
evaluate the model rate $a\!=\!a_{\max}S(\Gamma)$ at the $\theta$-scale amplifying
band and convert it to an envelope slope through the similarity geometry,
normalized once at the Blasius point (the same anchor used above).
Figure~\ref{fig:shapefactor} compares the result to the Drela--Giles
envelope-rate correlation $dn/dRe_\theta(H)$. The implied rate reproduces the
pressure-gradient (shape-factor) dependence over the attached range
$H\!\approx\!2.2$--$3.3$, from favorable ($H\!<\!2.6$) through mild adverse, from
the single Blasius calibration alone; near separation ($H\!\gtrsim\!3.5$) it
overshoots the correlation as $\Gamma_\mathrm{band}\!\to\!1$ drives the sigmoid
toward the free-shear ceiling $a_{\max}$---the inflectional/Kelvin--Helmholtz
regime the ceiling was set for.

\begin{figure}[tp]
  \centering
  \includegraphics[width=0.58\textwidth]{shapefactor_amplification.pdf}
  \caption{Shape-factor dependence of the amplification rate. For each
  Falkner--Skan profile (shape factor $H$) the SA-AI rate $a\!=\!a_{\max}S(\Gamma)$
  at the $\theta$-scale band is converted to an envelope slope $dN/dRe_\theta$ via
  the similarity geometry and normalized once to the Drela--Giles envelope at the
  Blasius point (red, the anchor of Sec.~\ref{sec:calib_rate}). Away from that
  single anchor the implied rate tracks the Drela--Giles correlation
  $dn/dRe_\theta(H)$~\cite{drela_giles_1987} across the attached range; near
  separation (shaded, $H\!\gtrsim\!3.5$) it overshoots as the sigmoid saturates to
  the free-shear ceiling $a_{\max}$.}
...
$100$ is a round, empirically chosen figure, not a finely tuned constant.
...
(Eq.~\eqref{eq:cliff_fpg}) to delay onset there. The floor likewise suppresses the
near-wall region automatically, where $\Gamma\!\to\!1$ but $Re_\Omega\!\to\!0$.
...
value $K_\lambda\!=\!10$; its magnitude is set by the Drela--Giles envelope and the
Falkner--Skan family, and a more careful calibration is left to future work.
...
\subsection{Cross-stream diffusion, handover, and the turbulent layer}\label{sec:calib_diff}
...
$c_{\nu,\mathrm{ai}}\!=\!1/12$ (determined below)---on the Blasius layer, seeded
...
coupled solver (Sec.~\ref{sec:flatplate}). The seed is well posed as an inflow
condition: in the irrotational outer flow the production vanishes with the
vorticity and the destruction is gated off in the laminar range ($\sigma_t\!=\!0$
...
solution, not chosen a priori. With the unmodified value
...
This reduction is invisible to Spalart's turbulent calibrations. The reason is structural: $c_{\nu,\mathrm{ai}}$ rescales
...
($c_{\nu,\mathrm{ai}}\!=\!1/12$, gate held at $\sigma_t\!\equiv\!1$) leaves the
...
$\tilde\nu$ profile---and with it $\nu_t\!=\!\rho\tilde\nu f_{v1}$, the quantity
the momentum equation feels---is unmoved by even a twelvefold reduction of the
molecular coefficient (Fig.~\ref{fig:walllayer}).

The handover introduces one coefficient of its own, the width $\tau$ in
$\sigma_t(\chi)\!=\!1-e^{-(\chi-1)/\tau}$. We set it \emph{ab initio} rather than
by tuning: the handoff begins as soon as $\chi$ exceeds $1$---where $\tilde\nu$
starts to feed the eddy viscosity---and is essentially complete by the scale
$\chi\!\sim\!c_{v1}$ at which $f_{v1}$ switches the eddy viscosity fully on, a span
that $\tau\!=\!4$ realizes. It was not optimized; future tuning of $\tau$ could
refine the handoff, with the trade-off quantified next.

The modification that \emph{does} leave a turbulent-wall footprint is the
$\sigma_t$ gate on production and destruction, not the diffusion. Because
$\sigma_t(\chi)\!=\!1-e^{-(\chi-1)/\tau}$ approaches unity only for
$\chi\!\gtrsim\!20$ ($y^+\!\gtrsim\!50$, where $\chi\!\approx\!\kappa y^+$), the SA
...
the gate merely scales the recovered SA source by $\sigma_t\!<\!1$. The same
wall-layer solve isolates this effect (Fig.~\ref{fig:walllayer}): gating $P$ and
$D$ lowers the log-law intercept $B$ by $\approx\!0.3$, a $\sim\!2$--$3\%$ rise in
skin friction $C_f$. It
is a \emph{uniform}, gradient-independent offset---a rigid downshift of $B$, not a
distortion of the profile---comparable to the $2$--$5\%$ scatter of the flat-plate
data on which SA was founded, and absorbable into an effective log-law constant. A
sharper handover (a smaller $\tau$, so $\sigma_t$ saturates ahead of the buffer
layer) would remove it at the price of a more abrupt laminar$\to$turbulent switch;
balancing the two is part of the coefficient tuning left to future work.

The fixed offset aside, does the modification change SA's tolerance to
...
$8$, geometric growth to the edge) answers this directly (Table~\ref{tab:yplus}).
...
$y^+\!=\!8$. The modification shifts the log-law intercept by the fixed
$\Delta B\!\approx\!-0.34$ but leaves the mesh sensitivity itself untouched: SA-AI
...
\begin{table}[tp]
  ...
  sensitivity matches standard SA's at every resolution; the modification adds only
  the fixed offset $\Delta B\!\approx\!-0.34$.}
  \label{tab:yplus}
  \begin{tabular}{cc cc cc}
    ...
    \midrule
    0.25 & 38 & 5.16 & ref    & 4.82 & ref \\
    0.5  & 34 & 5.17 & 0.1\%  & 4.83 & 0.1\% \\
    1    & 31 & 5.18 & 0.2\%  & 4.84 & 0.2\% \\
    2    & 27 & 5.21 & 0.4\%  & 4.87 & 0.5\% \\
    4    & 23 & 5.26 & 0.9\%  & 4.91 & 0.8\% \\
    8    & 19 & 5.44 & 2.5\%  & 5.13 & 2.7\% \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{figure}[tp]
  ...
  (solid) and SA-AI (dashed), with $\kappa y^+$ (dotted): the two coincide wherever
  $\nu_t^+$ is dynamically
  significant ($y^+\!\gtrsim\!7$, above the molecular level $\nu^+\!=\!1$),
  differing only deep in the viscous sublayer where $\nu_t^+\!\ll\!\nu^+$ and the
  eddy viscosity is inert. \textbf{(b)} The difference normalized by the total
  effective viscosity the momentum equation feels,
  $\Delta\nu_t^+/(\nu_t^+\!+\!\nu^+)$: it peaks at only $\sim\!4.6\%$ in the buffer
  layer (shaded, where the $\sigma_t$ gate has $\sigma_t\!<\!1$) and vanishes in
  both the sublayer and the log layer. \textbf{(c)} The velocity profiles $u^+$
  (nearly coincident, left axis) and their difference
  $\Delta u^+\!=\!u^+_{\mathrm{mod}}-u^+_{\mathrm{orig}}$ (dash-dot, right axis): the
  offset builds through the buffer and plateaus at the log-law intercept shift
  $\Delta B\!=\!-0.33$ ($\sim\!2$--$3\%$ in $C_f$). The reduced molecular diffusion
  \emph{alone} contributes $|\Delta B|\!<\!10^{-3}$, so this footprint is carried
  entirely by the $\sigma_t$ gate on production and destruction, not by the
  diffusion scaling.}
  \label{fig:walllayer}
\end{figure}
...
\begin{figure}[tp]
  ...
  $c_{\nu,\mathrm{ai}}\!=\!1/12$). Line contours show $\ln\hat\nu$ in the
...
$N_\mathrm{crit}=A+B\ln Tu$ ($A\!=\!-8.43$, $B\!=\!2.4$), used unchanged: the
SA-AI seed/threshold structure mirrors the $e^N$ envelope/$N_\mathrm{crit}$ split,
so the same map applies---the amplification transport plays the role of the $e^N$
envelope, and $N_\mathrm{crit}=\ln(c_{v1}/\chi_\infty)$ the threshold---which the
flat plate confirms to within $\sim\!8\%$ (Sec.~\ref{sec:flatplate}). The
...
For reference, the complete model in one place, in the order equation, terms,
constants. The working variable $\tilde\nu$ obeys the single transport equation
\begin{equation}
  \frac{D\tilde\nu}{Dt} = P - D
  + \frac{1}{\sigma}\nabla\!\cdot\!\big[(c_{\nu,\mathrm{ai}}\,\nu+\tilde\nu)\,\nabla\tilde\nu\big]
  + \frac{c_{b2}}{\sigma}\,|\nabla\tilde\nu|^2,
...
factor $c_{\nu,\mathrm{ai}}$ on the molecular diffusion. No transport equation is
added, and $\sigma_t\!\equiv\!1$, $c_{\nu,\mathrm{ai}}\!=\!1$ recover baseline SA
exactly.

\paragraph{Terms.} The production and destruction are
\begin{equation}
  P = \max\!\big[\,(1-\sigma_t)\,a\,\omega\,\tilde\nu,\;\;\sigma_t\,P_\mathrm{SA}\,\big],
  \qquad
  D = \sigma_t\,D_\mathrm{SA},
  \label{eq:aisa_pd}
\end{equation}
with the handover $\sigma_t(\chi)\!=\!\max[\,1-e^{-(\chi-1)/\tau},\,0]$
(Eq.~\eqref{eq:sigma}) and the amplification rate $a(Re_\Omega,\Gamma,\lambda_p)$
...
standard near-wall limiter on $\tilde S$ and the negative-$\tilde\nu$ safeguards
of Ref.~\cite{spalart_allmaras_1992}.
...
\end{equation*}
The SA-AI constants, each anchored to a canonical stability limit rather than fit
to a transition case, are
\begin{equation*}
  \begin{aligned}
...
ballpark, while the digits of $(s,g_c)$ merely solve the two envelope anchors. The
...
Flow360, an unstructured, second-order finite-element solver with a Roe flux; the
...
flat plate at five freestream-turbulence levels spanning the natural-transition
...
(Sec.~\ref{sec:calib}), adopted. We compare the resulting transition onset to the Abu-Ghannam \&
...
log-law correlation (dotted). As $Tu$ is raised the seed grows, so
...
reference with a solid vertical line and a filled symbol on the bottom edge. The
...
closure taken directly from Mack, the AGS onset lies between these two
crossings, with the $\chi\!=\!1$ crossing tracking it to within $\sim\!7\%$ in
$Re_\theta$ through $Tu\!=\!0.30\%$ ($-0.2\%$ at $Tu\!=\!0.04\%$, $+7.7\%$ at the
...
constrained-Delaunay triangulation everywhere, the boundary layer included, with
no structured near-wall layer at all. There the metric is stretched by
...
\begin{figure}[tp]
  ...
  location), which converges at both; the continuous amplification envelope $N(x)$
...
turbulent level downstream, on both surfaces and at every operating point. Both
surfaces and both mesh families place the rise at the same chordwise location at
L1 and L2; the onset locations, and their comparison to the $e^9$ reference and
to the measured transition, are collected in Table~\ref{tab:nlftrans}. The
...
Table~\ref{tab:nlftrans} places these onset locations against the wind-tunnel
...
($0.55\!\to\!0.64\,c$) as the lift rises; the $e^9$ panel solver sits
$\sim\!0.03$--$0.07\,c$ downstream of both. On the upper surface the model
essentially recovers the measured onset at $\alpha\!=\!0^\circ$
(SA-AI $0.37\,c$, experiment $0.38\,c$) and marches the front forward with
incidence---to $0.26\,c$ at $\alpha\!=\!4^\circ$, against a measured front
...
to roughly a third-chord by $\alpha\!=\!8^\circ$ ($c_l\!=\!1.3$). mfoil converges
only through $\alpha\!=\!7^\circ$: it fails to converge at $\alpha\!=\!9^\circ$ and
stalls at $15^\circ$. XFOIL~\cite{drela_xfoil_1989}, the reference $e^9$ solver,
converges at both and supplies the $e^9$ comparison there
(Table~\ref{tab:nlftrans}); its transition locations agree with the present RANS
to within $\sim\!0.03\,c$.
...
\begin{table}[tp]
  ...
  model, solver-reported onset on the finest structured O-grid (L2); the cavity
  L2 grid agrees to within $0.01\,c$ except at $\alpha\!=\!9^\circ$ upper
  (marginal near-LE onset, $0.04\,c$). \emph{mfoil}: $e^9$ coupled
  viscous--inviscid panel solver~\cite{fidkowski_mfoil_2022}. \emph{Exp}: Somers
  ...
  $^\dagger$From XFOIL~\cite{drela_xfoil_1989}: mfoil does not converge at
  $\alpha\!=\!9^\circ$ and converges only to a stalled state at $15^\circ$,
  whereas XFOIL---the reference $e^9$ solver---converges at both.}
  \label{tab:nlftrans}
  \begin{tabular}{cc ccc c ccc}
    \toprule
    & & \multicolumn{3}{c}{Upper $x_\mathrm{tr}/c$} & & \multicolumn{3}{c}{Lower $x_\mathrm{tr}/c$}\\
    \cmidrule(lr){3-5}\cmidrule(lr){7-9}
    $\alpha$ & $c_l$ & SA-AI & mfoil & Exp & & SA-AI & mfoil & Exp \\
    \midrule
    $0^\circ$  & 0.49 & 0.37 & 0.43 & 0.38 & & 0.55 & 0.62 & 0.55 \\
    $4^\circ$  & 0.95 & 0.26 & 0.33 & $0.31^\ddagger$ & & 0.60 & 0.65 & 0.62 \\
    $9^\circ$  & 1.48 & 0.08 & $0.09^\dagger$ & ---  & & 0.61 & $0.67^\dagger$ & 0.64 \\
    $15^\circ$ & 1.94 & 0.00 & $0.00^\dagger$ & ---  & & 0.64 & $0.69^\dagger$ & $0.66^\ddagger$ \\
    \bottomrule
  \end{tabular}
\end{table}
...
the front. A convex blend $\sigma_t P_\mathrm{SA}+(1-\sigma_t)P_\mathrm{AI}$
would leave a $(1-\sigma_t)P_\mathrm{AI}$ bleed there, keeping the
...
$\Gamma$-sigmoid centered at $g_c\!=\!1.572$, the amplification rate falls more than
twentyfold ($a\!\approx\!0.013\!\to\!0.0005$), switching growth off. The effect is
...
\begin{figure}[tp]
  ...
  XFOIL's (grey diamonds), which converges there.}
  \label{fig:nlfpolar}
\end{figure}
...
(under-resolved transition front). The mfoil $e^9$ reference tracks the same
low-drag bucket through $\alpha\!=\!7^\circ$ ($C_l\!=\!1.33$) but provides no
valid point at higher incidence: its coupled iteration does not converge at
$\alpha\!=\!9^\circ$ and converges only to a stalled solution at
$\alpha\!=\!15^\circ$. This is specific to mfoil, not to the setup or the method
class: the runs use the identical NLF(1)-0416 coordinates,
$Re\!=\!4\!\times\!10^6$, $M\!=\!0.1$, and $N_\mathrm{crit}\!=\!9$ as the RANS
cases, and XFOIL~\cite{drela_xfoil_1989}---the reference coupled
inviscid--viscous $e^N$ solver---converges at the same conditions
($C_l\!=\!1.52$ at $\alpha\!=\!9^\circ$, consistent with the present RANS), so
the failure lies in mfoil's Newton globalization rather than in the underlying
method.
...
\begin{figure}[tp]
  ...
  dotted, and the tabulated experimental $C_p$ (NASA TM-4062 App.~D, nearest-$\alpha$
  column) as open black circles on the $-C_p$ row). The signed $C_f$ (row~5) shows the laminar-separation bubble
...
\begin{figure}[tp]
  ...
  $\alpha\!=\!7^\circ$ the dotted reference is XFOIL rather than mfoil: mfoil
  stalls numerically there ($C_l\!=\!0.928$, below its $\alpha\!=\!5^\circ$
  value; cf.\ Table~\ref{tab:eppxtr}), whereas XFOIL converges cleanly
  ($C_l\!=\!1.17$). XFOIL provides $C_p$ and $C_f$ only (no amplification
  envelope), so the mfoil $N$ curve (row~3) is absent at $7^\circ$.}
  \label{fig:eppcfhigh}
\end{figure}
...
it node-for-node), directly comparable to mfoil's signed skin friction. Both Flow360
...
\begin{table}[tp]
  ...
  \begin{tabular}{ccccccccc}
    ...
    \midrule
    $0^\circ$ & 0.731 & 0.721 & 0.724 & 0.703 & 0.722 & 0.729 & 0.764 & 0.74 \\
    $2^\circ$ & 0.680 & 0.676 & 0.677 & 0.657 & 0.673 & 0.680 & 0.713 & 0.67 \\
    $5^\circ$ & 0.617 & 0.599 & 0.598 & 0.590 & 0.593 & 0.600 & 0.612 & 0.59 \\
    $7^\circ$ & 0.493 & 0.466 & 0.472 & 0.323 & 0.433 & 0.477 & $0.40^\dagger$ & 0.48 \\
    \bottomrule
  \end{tabular}
...
The reattachment locations in Table~\ref{tab:eppxtr} converge monotonically on both
mesh families: the L1$\to$L2 drift is $\leq\!0.01\,c$ at
$\alpha\!\in\!\{0^\circ,2^\circ,5^\circ\}$, and the two independently generated
meshes agree to within $0.005\,c$ at L2 on all four incidences. The
$\alpha\!=\!7^\circ$ case is the most sensitive, as expected near stall---the
structured L0 reattaches $\sim\!0.15\,c$ too early---but L1 and L2 recover the
same $x_R\!\approx\!0.47$--$0.48$. Against the oil-flow data the agreement is
close at every incidence: SA-AI (L2) matches the measured reattachment to within
$\sim\!0.01$--$0.02\,c$ ($0.72$--$0.73$ vs $0.74$ at $\alpha\!=\!0^\circ$, $0.68$
vs $0.67$ at $2^\circ$, $0.60$ vs $0.59$ at $5^\circ$, and $0.47$--$0.48$ vs
$0.48$ at $7^\circ$), sitting marginally upstream. The mfoil $e^9$ reference
...
The $Re\!=\!2\!\times\!10^5$ benchmark fixes the model; the remaining question is
whether the same calibration follows the bubble across the Reynolds-number range
of the McGhee experiment without retuning. We hold the incidence at
...
on the L1 grids of both mesh families.
...
(open circles, hand-read from the NASA TM-4062 Appendix~D tables at the
nearest-$5^\circ$ column) and the mfoil/XFOIL $e^9$ reference where it converges.
...
$x/c\!\approx\!0.4$ and does not recover ahead of the trailing edge, the
low-$Re$ bubble-bursting limit---then a long bubble reattaching near
$x/c\!\approx\!0.73$ at $10^5$, and progressively shorter bubbles reattaching at
$x/c\!\approx\!0.55$ ($3\!\times\!10^5$) and $x/c\!\approx\!0.50$
($4.6\!\times\!10^5$). The section drag falls monotonically across the range,
$C_d\!=\!0.040,\,0.021,\,0.011,\,0.0097$ (unstructured L1), capturing the sharp
low-$Re$ drag rise as the bubble lengthens toward bursting. The measured
...
The two independently generated L1 meshes bracket the same solution at the higher
Reynolds numbers ($\Delta C_d\!\lesssim\!4\!\times\!10^{-4}$ at
$3$--$4.6\!\times\!10^5$) and diverge only at $6\!\times\!10^4$, where the open
separation is genuinely mesh-sensitive ($C_l\!=\!0.80$ unstructured vs.\ $0.73$
structured)---the expected behavior once the bubble fails to close and the
afterbody is fully separated. The mfoil $e^9$ panel reference converges from
...
XFOIL~\cite{drela_xfoil_1989} supplies the $e^9$ line instead; both the
viscous-panel reference and the measured pressures track the computed
distributions. Across the full factor-of-eight Reynolds-number range the model
reproduces the bubble march with the single $Re\!=\!2\!\times\!10^5$ calibration
left untouched.
...
\begin{figure}[tp]
  ...
  nearest-$5^\circ$ column). Grey band: experimental laminar-separation-bubble
...
without any auxiliary field. Because the amplification rate depends only on the local mean flow
and not on the working variable, the source is local and continuous (the
production blend is a $\max$, hence continuous but not everywhere differentiable).
...
closure $\chi_\infty(Tu)$ is Mack's $e^N$ correlation. On a zero-pressure-gradient flat plate (Sec.~\ref{sec:flatplate}) this
...
$Re\!=\!2\!\times\!10^5$ (Sec.~\ref{sec:eppval}), with the same constants, the
...
two-family ladder. The same unified constants (Sec.~\ref{sec:assembled}) were used
