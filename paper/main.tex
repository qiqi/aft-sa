...
favorable-pressure-gradient suppression is handled separately by the
$\lambda_p$ sensor of Eq.~\eqref{eq:indicators} (see below), avoiding the
contamination of Blasius growth that a $\Gamma$-dependent (slanted) cliff would
otherwise cause. The second term in $z$ is the
...
pressure-gradient sensor $\lambda_p$ of Eq.~\eqref{eq:indicators} that suppresses
...
favorable-pressure-gradient suppression built on $\lambda_p$ of
Eq.~\eqref{eq:indicators},
\begin{equation}
  \sigma_\mathrm{FPG}(\lambda_p) =
       \frac{1}{1+\exp(s_\lambda(\lambda_p-\lambda_\ast))} .
  \label{eq:fpg}
\end{equation}
The suppression is calibrated against Falkner--Skan profiles at
$\eta\!\approx\!1$ (the kernel-active band): Blasius gives $\lambda_p\!=\!0$ and
Hiemenz ($\beta\!=\!1$) gives $\lambda_p\!\approx\!1.29$, so anchoring
$\sigma_\mathrm{FPG}(0)\!=\!0.95$ and $\sigma_\mathrm{FPG}(1.29)\!=\!0.05$ yields
$(\lambda_\ast, s_\lambda)\!=\!(0.64, 4.56)$. An adverse gradient
($\lambda_p\!<\!0$) then sees $\sigma_\mathrm{FPG}\!>\!0.95$, leaving the
inflectional-profile amplification---which the $\Gamma$-sigmoid already
captures---untouched. Like the other indicators \eqref{eq:indicators},
$\lambda_p$ is local and independent of $\tilde\nu$, so it preserves the
analytic Jacobian of Sec.~\ref{sec:impl}.
...
(log); \emph{row 2} the favorable-pressure-gradient suppression
$\sigma_\mathrm{FPG}$ of Eq.~\eqref{eq:fpg}, evaluated at the wall-normal
location where $Re_\Omega$ peaks (the kernel-active band the amplification
actually sees --- the quantity tuned in this section), with
$\sigma_\mathrm{FPG}\!\to\!0$ where the flow accelerates near the leading edge
and $\to\!1$ once the favorable gradient relaxes; \emph{row 3} the in-BL maximum
...
\begin{figure}[tp]
  ...
  row~2 $\sigma_\mathrm{FPG}$, row~3 $\chi$ with mfoil $N$, row~4 $-C_p$,
...
\begin{figure}[tp]
  ...
  described in the text (row~1 $\max Re_\Omega$, row~2 $\sigma_\mathrm{FPG}$,
