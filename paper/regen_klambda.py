...
S_SLOPE, G_C, P, FLOOR, CA = 11.0, 1.005, 4.0, 254.0, 4.0
...
def prof(beta):
    ...
    A4 = CA*(fppp*eta**2)**2
    Q = 1.0 - 2.0*np.abs(fpp*eta)*np.abs(fp)/(A4 + (fpp*eta)**2 + fp**2 + 1e-30)
    return dict(ReO=ReO, G=G, lam=lam, Q=Q, H=H, Rc=Rc)

def worst_lam(pr, KL):
    S = pr['Q']*Sz(pr['ReO']/(FLOOR*np.exp(KL*np.maximum(pr['lam'], 0))), pr['G'])
    if S.max() <= 0:  # touching limit: barrier dominates, argmax of ReO/cliff
        r = pr['ReO']/(FLOOR*np.exp(KL*np.maximum(pr['lam'], 0)))
        return int(np.argmax(r)), S
    return int(np.argmax(S)), S

# ---------- FIG 1: profiles at Drela-critical Re, colored by cliff clearance ----------
KL = 6.1
betas = [-0.15, -0.10, 0.0, 0.3, 0.6, 1.0, 1.4]
fig, ax = plt.subplots(figsize=(7.4, 5.1))
norm = plt.Normalize(-2.0, 2.0)   # log2 of clearance r = Re_Omega / cliff(lambda_p)
for b in betas:
    pr = prof(b); ReO, G, lam = pr['ReO'], pr['G'], pr['lam']
    cliff = FLOOR*np.exp(KL*np.maximum(lam, 0))
    r = np.maximum(ReO, 1e-12)/cliff
    ...
    i = int(np.argmax(r)); ax.plot(ReO[i], G[i], '*', ms=14,
                                   mfc=cm.coolwarm(norm(c[i])), mec='k', mew=0.7)
    j = int(np.argmax(ReO)); ax.annotate(fr'$\beta$={b}', (ReO[j], G[j]), fontsize=7.5, ha='center', va='bottom')
ax.axvline(FLOOR, color='0.4', ls=':', lw=1.1)
ax.text(FLOOR*1.07, 1.45, r'floor $Re_\Omega^{\mathrm{f}}=254$', fontsize=8, color='0.3', rotation=90, va='top')
...
Ku = np.linspace(3.5, 8.5, 26); Ki = []      # fixed point sits near 5.9 at this kernel
for K in Ku:
