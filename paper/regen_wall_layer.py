...
Two solutions, obtained by continuation (blend lam: 0 -> 1):
    lam=0 : original SA
    lam=1 : SA-AI  (production & destruction gated by sigma_t, molecular
                    diffusion coefficient reduced by c_nu,aft = 1/12)
...
def rhs(e, y, lam):
    ...
    s = 1.0 - lam*(1.0 - sigt(N))                 # gate P,D by sigma_t  (lam: 0->1)
    dc = (1.0 + N) - lam*(1.0 - CNU)              # reduce molecular diffusion
    return np.vstack([Np, (-sig*s*(P - D) - (1 + cb2)*Np**2)/dc])
...
print(f"B0={B0:.3f}  B1={B1:.3f}  dB={dB:+.3f}")
...
axD.axhline(dB, color='0.65', ls=':', lw=1.0)
axD.set_ylim(-0.5, 0.06)
