...
(default: the shipped ``sa-ai/flow360_fr/``). Two preparation steps are
...
1. **Tree assembly** -- symlink every canonical case name into one root.
   ...
   not re-run falls back to the shipped ``flow360_g4`` entry (e.g. the fully-turbulent baselines, which the transition kernel does not touch), so a partial
...
SAAI = Path(__file__).resolve().parent.parent.parent.parent
SHIPPED = SAAI / "flow360_g4"
