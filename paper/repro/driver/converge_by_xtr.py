...
sys.path.insert(0, os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', 'cfd'))
...
def main():
    ...
    ap.add_argument('--min-batches', type=int, default=0,
                    help='ignore stability for the first N batches of this invocation: '
                         'fresh impulsive starts sit on flat sentinel plateaus for 2-4 '
                         'batches (all-turbulent (0,0) at high alpha, all-laminar '
                         '(~0.99,~0.99) at low alpha) before the true front appears, '
                         'which consec alone mistakes for convergence')
    ...
    for it in range(args.max_batches):
        ...
        if len(hist) >= 2:
            ...
            # exact 0.0 = chi>1 already at the x=0 grid station: the impulsive-start
            # all-turbulent transient, never a true onset (converged LE onsets
            # interpolate to small-positive x, e.g. 3e-4). Don't count it as stable.
            sentinel = (hist[-1]['xtr_upper'] == 0.0 or hist[-1]['xtr_lower'] == 0.0)
            if it + 1 <= args.min_batches or sentinel:
                stable = 0
                why = 'min-batches' if it + 1 <= args.min_batches else 'sentinel 0.0'
                print(f"  stability not counted ({why})", flush=True)
            elif du < args.tol and dl < args.tol:
                stable += 1
