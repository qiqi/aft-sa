...
def compute_viscous_fluxes_tight_jax(
    ...
    For each face:
    1. Two-point difference along coordinate line: grad_coord = (phi_R - phi_L) / d_coord
    2. LS weighted sum for orthogonal correction: grad_ortho = sum(w_k * phi_k)
    3. Combine: grad_x = grad_coord * e_coord_x + grad_ortho * e_ortho_x
                grad_y = grad_coord * e_coord_y + grad_ortho * e_ortho_y
    4. Compute flux based on variable:
       - Index 0 (p): no diffusion
       - Index 1,2 (u,v): stress tensor with mu_eff
       - Index 3 (nuHat): SA diffusion with (nu + nuHat) / sigma
    ...
    mu_eff : effective viscosity for momentum
    nu_laminar : laminar kinematic viscosity
    nuHat : SA working variable
    sigma : SA model constant
    ...
    # Pad Q for stencil extraction (pad with edge values)
    Q_padded = jnp.pad(Q, ((1, 1), (1, 1), (0, 0)), mode='edge')  # (NI+2, NJ+2, 4)
    
    # Pad diffusion coefficients
