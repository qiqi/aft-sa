"""Run the TE-refined cavity meshes (L0, L1, L2) under SA-AI at α=4°.
"""
...
NGPU_BY_LEVEL = {'L0': 1, 'L1': 1, 'L2': 2}
CASES = [('proper_cav_L0_TE', 'L0'),
         ('proper_cav_L1_TE', 'L1'),
         ('proper_cav_L2_TE', 'L2')]
...
def run_one(case_dir, ngpu):
    ...
    # Use damped CFL for stability (same as L3 fresh run)
    ...
    print(f"  [2/3] MeshProcessor", flush=True)
    if ngpu == 1:
    ...
    print(f"  [3/3] Flow360Solver", flush=True)
    ...
    try:
        ...
        if ngpu == 1:
            ...
            senv["OMPI_COMM_WORLD_RANK"] = "0"; senv["OMPI_COMM_WORLD_SIZE"] = "1"
        ...
        while True:
            if solver.poll() is not None:
                ...
                print(f"    solver exit rc={rc}"); break
            try:
                ...
                if END_MARKER in tail and end_t is None:
                    end_t = time.time()
                    print(f"    end-marker at t={time.time()-t0:.0f}s", flush=True)
            except: pass
    ...
    try:
        ...
        print(f"  [{time.strftime('%H:%M:%S')}] step={step} cont={cont:.1e} nuHat={nuHat:.1e} CL={CL:.4f} CD={CD:.5f} t={time.time()-t0:.0f}s", flush=True)
...
for case_dir, level in CASES:
    ...
    r = run_one(case_dir, ngpu)
    if r: results[case_dir] = r

json.dump(results, open(f"{F}/te_refined_results.json", 'w'), indent=1)
