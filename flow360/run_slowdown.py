"""Test the in-equation laminar slowdown (AI_LAMINAR_SLOWDOWN=f) on the O-grid NLF a4 cycle,
using DEFAULT solver coupling (evalFreq=4, CFLmult=2). If f<1 kills the cycle and lands on the
ef50 fixed point (CD~0.0125, xtr~0.286), the model-embedded timescale separation works."""
...
def stage(wd,steps=40000,alpha=4):
    ...
    json.dump(c,open(f"{wd}/Flow360.json","w"),indent=1);return wd  # DEFAULT coupling (no evalFreq/CFLmult change)
def cf_upper_xtr(d):
...
def run(tag,fslow,gpu):
    with sem:
        try:
            wd=stage(f"{F}/slow_{tag}");env,find=make_env();env["AI_SA"]="1";env["AI_LAMINAR_SLOWDOWN"]=str(fslow)
            run_solver(wd,find,env,gpu=gpu,timeout=9000);m,sd,rc=stats(wd)
            try:xt=cf_upper_xtr(wd)
            except:xt=float('nan')
            with lock:res[tag]=dict(CDmean=round(m,5),CDstd_pct=round(100*sd/max(m,1e-9),2),cont=rc,xtr=round(xt,3))
            print("done f=%-6s CDmean=%.5f CDstd=%.2f%% xtr=%.3f cont=%.1e"%(tag,m,100*sd/max(m,1e-9),xt,rc),flush=True)
        except Exception as e:
            with lock:res[tag]=dict(err=str(e)[:70]);print("FAIL %s %s"%(tag,e),flush=True)
jobs=[("1.0",1.0),("0.1",0.1),("0.03",0.03),("0.01",0.01)]
ts=[threading.Thread(target=run,args=(t,fs,i%8)) for i,(t,fs) in enumerate(jobs)]
for t in ts:t.start()
for t in ts:t.join()
json.dump(res,open(f"{F}/run_slowdown_results.json","w"),indent=1);print("SLOWDOWN DONE:",json.dumps(res))
