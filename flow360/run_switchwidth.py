"""Test the user's gentle-handover hypothesis: widen is_turb (switchWidth) and see if the
O-grid NLF a4 limit cycle damps. switchWidth=4 is default; 8/16/32 = gentler handover."""
...
def stage(wd,steps=15000,alpha=4):
...
def run(sw,gpu):
    with sem:
        try:
            ...
            with lock:res[str(sw)]=dict(CDmean=round(m,5),CDstd_pct=round(100*sd/max(m,1e-9),1))
            print("done switchWidth=%-4s CDmean=%.5f CDstd=%.1f%%"%(sw,m,100*sd/max(m,1e-9)),flush=True)
        except Exception as e:
            with lock:res[str(sw)]=dict(err=str(e)[:70]);print("FAIL sw=%s %s"%(sw,e),flush=True)
jobs=[4,8,16,32]
ts=[threading.Thread(target=run,args=(sw,i%8)) for i,sw in enumerate(jobs)]
for t in ts:t.start()
for t in ts:t.join()
json.dump(res,open(f"{F}/run_switchwidth_results.json","w"),indent=1)
print("SWITCHWIDTH TEST DONE:",json.dumps(res))
