"""A/B test the two new levers on O-grid NLF a4 (DEFAULT solver coupling), GPUs 1-7 (avoid GPU0).
A = gated-max blend (AI_MAXBLEND=1). B = extended-slowdown gate (AI_SLOWWIDTH=30) at moderate f."""
...
def run(tag,extra_env,steps,gpu):
    with sem:
        try:
            wd=stage(f"{F}/blend_{tag}",steps);env,find=make_env();env["AI_SA"]="1"
            ...
            print("done %-22s CDmean=%.5f CDstd=%.2f%% xtr=%.3f cont=%.1e"%(tag,cd[m].mean(),100*cd[m].std()/cd[m].mean(),xt,float(rl[rh.index('0_cont')])),flush=True)
        except Exception as e:
            with lock:res[tag]=dict(err=str(e)[:70]);print("FAIL %s %s"%(tag,e),flush=True)
# (tag, env, steps)
jobs=[("control_convex",{},20000),
      ("A_maxblend",{"AI_MAXBLEND":1},25000),
      ("B_slowext_f0.1_w30",{"AI_LAMINAR_SLOWDOWN":0.1,"AI_SLOWWIDTH":30},40000),
      ("B_slowext_f0.03_w30",{"AI_LAMINAR_SLOWDOWN":0.03,"AI_SLOWWIDTH":30},40000),
      ("AB_max_slowext",{"AI_MAXBLEND":1,"AI_LAMINAR_SLOWDOWN":0.1,"AI_SLOWWIDTH":30},40000)]
ts=[threading.Thread(target=run,args=(t,e,st,1+(i%7))) for i,(t,e,st) in enumerate(jobs)]  # GPUs 1-7
for t in ts:t.start()
for t in ts:t.join()
json.dump(res,open(f"{F}/run_blend_results.json","w"),indent=1);print("BLEND TEST DONE:",json.dumps(res))
