"""Convert NLF Construct2D .p3d → Flow360 CGNS cases (L0/L1/L2 × α∈{0,4,9,15}).

Mirrors build_proper_struct_cases.py (NACA), but:
  - sources `proper_struct_nlf_LX.p3d` (built by build_nlf_proper_struct.py)
  - 4 angles of attack per level (instead of one)
  - patches Flow360.json to Re=4M / M=0.1 / chi_inf_input=8.76e-6 (compensated for fSlow=0.01)
  - adds residualTurbulence + residualNavierStokes + nuHat + wallDistance to slice + volume outputs

Output: 12 case dirs `strL{0,1,2}prop_nlf0416_Re4M_a{0,4,9,15}` ready for run_with_slowdown.py.
"""
...
CHI_INPUT = 8.76e-6   # compensated for AI_LAMINAR_SLOWDOWN=0.01 → actual chi_inf=8.76e-4 (N_crit=9)
...
def patch_flow360_for_nlf(cfg_path, alpha):
    d = json.load(open(cfg_path))
    d['freestream']['Mach'] = 0.1
    d['freestream']['muRef'] = 2.5e-8
    d['freestream']['alphaAngle'] = alpha
    # chi_inf compensated for the slowdown drift
    ...
    # sutherland constant for compressible viscosity (gets stripped by translator otherwise)
...
ncells_record = {}
for tag in LEVELS:
    p3d = f"{C2D_DIR}/proper_struct_nlf_{tag}.p3d"
    if not os.path.exists(p3d):
        print(f"SKIP {tag}: no {p3d}"); continue
    for alpha in ALPHAS:
        case_dir = f"{OUT_BASE}/strL{tag[1:]}prop_nlf0416_Re4M_a{int(alpha)}"
        print(f"\n=== building struct case {tag} alpha={alpha} → {os.path.basename(case_dir)} ===", flush=True)
        t = time.time()
        ...
        env, find = make_env()
        _mesh.gmsh_to_cgns(case_dir + "/mesh.msh", case_dir + "/mesh.cgns", find("flow360gmshtocgns"), env)
        ...
        # Patch the now-generated Flow360.json for NLF (Mach/muRef/chi_inf/outputs)
        patch_flow360_for_nlf(f"{case_dir}/Flow360.json", alpha)
        ncells_record[f"{tag}_a{int(alpha)}"] = n_quads
        print(f"  done {tag} α={alpha}: {n_quads} quads, t={time.time()-t:.0f}s, cum={time.time()-t_total:.0f}s", flush=True)

json.dump(ncells_record, open(f"{OUT_BASE}/nlf_struct_case_ncells.json", 'w'), indent=1)
print(f"\nTotal time: {time.time()-t_total:.0f}s")
print(f"NLF struct case cell counts: {ncells_record}")
