"""Parallel runner for the cavprop_* α-sweep: dispatches all pending cases
concurrently, one case per GPU. Each case uses CUDA_VISIBLE_DEVICES=<gpu_idx>.

Skips cases already in cavprop_alphasweep_results.json.
"""
...
ALPHAS = [-2, 0, 2, 4, 6, 8]
ALL_CASES = []
for a in ALPHAS: ALL_CASES.append((f"cavprop_naca0012_a{a}", 'aftsa'))
for a in ALPHAS: ALL_CASES.append((f"cavprop_nlf0416_a{a}",  'aftsa'))
for a in [0, 4]:  ALL_CASES.append((f"cavprop_naca0012_turb_a{a}", 'turb'))
...
print(f"already done: {sorted(done.keys())}")
CASES = [(c, t) for (c, t) in ALL_CASES if c not in done]
print(f"pending {len(CASES)} cases:")
for c, t in CASES: print(f"  {c} ({t})")
...
def run_one(case_dir, tag, gpu_idx):
    d = f"{F}/{case_dir}"
    if not os.path.exists(d): print(f"[gpu{gpu_idx}] SKIP missing {case_dir}"); return None
    ...
    if r.returncode!=0:
        print(f"[gpu{gpu_idx}] PARTITION FAIL: {r.stderr[-500:]}"); return None
    r = subprocess.run([find("MeshProcessor"),"--threads","1","mesh.cgns"],
                      cwd=d, env=env, capture_output=True, text=True, timeout=1800)
    if r.returncode!=0:
        print(f"[gpu{gpu_idx}] MeshProcessor FAIL: {r.stderr[-1000:]}"); return None
    ...
    try:
        ...
        while True:
            if solver.poll() is not None:
                print(f"[gpu{gpu_idx}] solver exit rc={solver.returncode}"); break
            try:
                ...
                if END_MARKER in tail and end_t is None:
                    end_t = time.time()
                    print(f"[gpu{gpu_idx}] {case_dir} end-marker at t={time.time()-t0:.0f}s", flush=True)
            except: pass
    ...
    try:
        ...
        print(f"[gpu{gpu_idx}] DONE {case_dir} step={step} cont={cont:.1e} CL={CL:.4f} CD={CD:.5f} t={time.time()-t0:.0f}s", flush=True)
        return dict(step=step, cont=cont, nuHat=nuHat, CL=CL, CD=CD, elapsed=time.time()-t0, tag=tag)
    except Exception as e:
        print(f"[gpu{gpu_idx}] result-parse FAIL {case_dir}: {e}"); return None

# Worker thread: pull cases off queue, run on assigned GPU
...
for gpu in range(min(NGPU, len(CASES))):
    t = threading.Thread(target=worker, args=(gpu, q))
    t.start()
    threads.append(t)
...
save_results()
print(f"\nALL DONE in {time.time()-t_start:.0f}s ({len(done)}/{len(ALL_CASES)} cases)")
for c, r in sorted(done.items()):
