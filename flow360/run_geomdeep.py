...
def stage(wd,steps=55000,alpha=4):
    ...
    c=json.load(open(f"{wd}/Flow360.json"));c['freestream']['alphaAngle']=float(alpha);c['timeStepping']['maxPseudoSteps"]=55000
...
def run(tag,fslow,gpu):
    with sem:
        try:
            wd=stage(f"{F}/geomdeep_{tag}");env,find=make_env();env["AI_SA"]="1";env["AI_LAMINAR_SLOWDOWN"]=str(fslow)
...
ts=[threading.Thread(target=run,args=(t,fs,i)) for i,(t,fs) in enumerate([("g0.01",0.01),("g0.005",0.005)])]
for t in ts:t.start()
for t in ts:t.join()
json.dump(res,open(f"{F}/run_geomdeep_results.json","w"),indent=1);print("GEOMDEEP DONE:",json.dumps(res))
