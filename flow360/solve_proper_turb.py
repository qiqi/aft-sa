"""Solve SA-turbulent on the new properly-resolved cavity at a=0 for Table 1."""
        ...
        try:
            while True:
                ...
                if time.time() - start > MAX_WALL_SEC:
                    ...
                    raise TimeoutError("MAX_WALL_SEC")
                time.sleep(1.0)
        finally: slog.close()
    finally:
        pp.terminate()
...
d = f"{F}/proper_naca0012_cavity_turb_a0"
print(f"solving {d} ...", flush=True)
# clean
for f in os.listdir(d):
    p = os.path.join(d, f)
    if (f.endswith(('.pvtu','.vtu','.pvd','.sock','.gltf')) or f.endswith('_v2.csv')
        or f.startswith('surface_forces') or f.endswith('.log') or f.endswith('.csv.bk')):
        try: os.remove(p)
        except: pass
...
env, find = make_env()
env.pop("AI_SA", None)  # important: SA-turbulent, not SA-AI
t0 = time.time()
try: run_watchdog(d, find, env, gpu=0)
except Exception as e: print(f"wd: {e}", flush=True)
print(f"  solver done in {time.time()-t0:.0f}s", flush=True)
...
CD = float(tl[th.index('CD')]); CDp = float(tl[th.index('CDPressure')])
print(f"SA-TURB cavity a=0: step={step} cont={cont:.1e} nuHat={nuHat:.1e}", flush=True)
print(f"  CD={CD:.5f} CDp={CDp:.5f} CDf={CD-CDp:.5f}", flush=True)
