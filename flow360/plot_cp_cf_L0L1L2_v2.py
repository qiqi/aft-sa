"""Cp+Cf upper & lower surface, plus CL/CD/CM grid convergence."""
...
def contour_walk_both(d, af='naca0012'):
    ...
    # decide which is upper (higher mean z)
...
CASES = [
    ('proper_cav_L0', 'cavity L0',  'k',  '-', 27796),
    ('proper_cav_L1', 'cavity L1',  'C0', '-', 96950),
    ('proper_cav_L2', 'cavity L2',  'C2', '-', 351314),
    ('proper_str_L0', 'O-grid L0',  'k',  '--', 15721),
    ('proper_str_L1', 'O-grid L1',  'C0', '--', 63441),
    ('proper_str_L2', 'O-grid L2',  'C2', '--', 254881),
]
...
plt.savefig('/tmp/cp_cf_L0L1L2_v2.png', dpi=140)
plt.savefig('/home/qiqi/flexcompute/aft-sa/paper/figs/proper_cp_cf_upper_lower.pdf')
print("wrote /tmp/cp_cf_L0L1L2_v2.png + paper figs/proper_cp_cf_upper_lower.pdf")
...
plt.tight_layout(pad=0.5)
plt.savefig('/tmp/force_convergence.png', dpi=140)
plt.savefig('/home/qiqi/flexcompute/aft-sa/paper/figs/proper_force_convergence.pdf')
print("wrote /tmp/force_convergence.png + paper figs/proper_force_convergence.pdf")

print("\n=== convergence ratios (L1->L2 vs L0->L1) ===")
def ratio(arr):
    if len(arr) < 3: return float('nan')
    return abs(arr[2]-arr[1])/max(abs(arr[1]-arr[0]), 1e-30)
print(f"  cavity: dCL_L1L2/dCL_L0L1 = {ratio(CL_cav):.2f}; dCD = {ratio(CD_cav):.2f}; dCMy = {ratio(CM_cav):.2f}")
print(f"  O-grid: dCL_L1L2/dCL_L0L1 = {ratio(CL_str):.2f}; dCD = {ratio(CD_str):.2f}; dCMy = {ratio(CM_str):.2f}")

print("\n=== cavity vs O-grid gap at each level ===")
print(f"{'level':>5s}  {'ΔCL/CL':>10s}  {'ΔCD/CD':>10s}  {'ΔCM/CM':>10s}")
for L, (c_CL, s_CL, c_CD, s_CD, c_CM, s_CM) in enumerate(zip(CL_cav, CL_str, CD_cav, CD_str, CM_cav, CM_str)):
    print(f"   L{L}    {100*(c_CL-s_CL)/s_CL:+9.3f}%   {100*(c_CD-s_CD)/s_CD:+9.3f}%   {100*(c_CM-s_CM)/s_CM:+9.3f}%")
