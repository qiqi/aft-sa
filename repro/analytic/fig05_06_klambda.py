...
Derivation of K_lambda from the worst-point self-consistency. All kernel constants
(s, g_c, p, floor, c_A) and K_lambda imported; Drela critical Re from src.correlations."""
import _saai
from _saai import S_SLOPE, G_C, P, FLOOR, C_A, K_LAMBDA
from src.physics.correlations import Re_theta0
import numpy as np
...
def main():
    ...
    for b in betas:
        ...
        i = int(np.argmax(r)); ax.plot(ReO[i], G[i], '*', ms=14, mfc=cm.coolwarm(norm(c[i])), mec='k', mew=0.7)
